...
func TestAgentNativeSessionStartupIsImmediatelyUsableAndIndexed(t *testing.T) {
	...
	if started.Operation == nil || started.Operation.State != "completed" || started.Runtime.NativeSession == nil || started.Session.Native == nil || started.Session.Status != "available" {
	...
	if err != nil || stored.Native == nil || stored.Native.ID != started.Runtime.NativeSession.ID || stored.Attempt.State != "ready" {
		t.Fatal("start required discovery to capture its session", err)
	}
...
func TestAgentNativeSessionQueueRetainsOperationAndCapturesOnlyConfirmation(t *testing.T) {
	...
	started, _ := directoryACPWithEnvironment(t, f, map[string]string{"DUNE_HOST_FAKE_ACP_GATE": gate})
	page, err := f.app.AgentDirectory().List(t.Context(), f.agentScope(), runner.Query{})
	if err != nil || len(page.Items) != 1 {
		t.Fatal(page, err)
	}
	...
	stored, err := f.app.store.AgentSession(t.Context(), f.owner, started.Session.ID)
	if err != nil || stored.Native.ID != started.Session.Native.ID || !stored.Selected {
		t.Fatal("queued load changed the recovery index", err)
	}
	if err := os.WriteFile(gate, nil, 0600); err != nil {
		t.Fatal(err)
	}
	completed, err := messenger.Wait(t.Context(), f.agentScope(), agents.WaitRequest{OperationRef: load.Ref, TimeoutMS: 3000})
	if err != nil || completed.Operation.State != "completed" || completed.Operation.NativeSession.ID != "saved-conversation" || completed.Operation.RecoveryError != "" {
		t.Fatal("load confirmation was not captured", completed, err)
	}
	current, err := f.app.store.RuntimeAgentSession(t.Context(), f.owner, agent.Target)
	if err != nil || current.ID == started.Session.ID || current.Native.ID != "saved-conversation" || current.Native.Cwd != "/native/work" {
		t.Fatal("new/load required discovery to save the correct session", err)
	}
...
func TestAgentNativeSessionStartupFailurePreservesRuntimeAndOperation(t *testing.T) {
	for _, outcome := range []string{"failed", "unknown"} {
		t.Run(outcome, func(t *testing.T) {
			...
			if errorCode(err) != code || result.Runtime == nil || result.Operation == nil || result.Operation.State != outcome || result.Session.Native != nil {
