...
func TestAgentMessengerUsesFabricdOperationsAcrossConnections(t *testing.T) {
	...
	first, err := firstService.Prompt(t.Context(), f.agentScope(), agents.PromptRequest{AgentRef: agent.Ref, Text: "first", WaitMS: 1})
	if err != nil || first.Terminal() || !strings.HasPrefix(first.Ref, "operation_") {
		t.Fatal("first submission", first, err)
	}
	second, err := secondService.Prompt(t.Context(), f.agentScope(), agents.PromptRequest{AgentRef: agent.Ref, Text: "second"})
	...
	_, err = firstService.Prompt(t.Context(), f.agentScope(), agents.PromptRequest{AgentRef: agent.Ref, Text: "must not be redirected"})
	...
	if _, err := secondService.Prompt(t.Context(), wrong, agents.PromptRequest{AgentRef: agent.Ref, Text: "cross tenant"}); !errors.Is(err, metadata.ErrNotFound) {
...
func TestAgentMessengerPTYDeliveryAndSnapshotRemainDistinct(t *testing.T) {
	...
	op, err := messenger.Prompt(t.Context(), f.agentScope(), agents.PromptRequest{AgentRef: agent.Ref, Text: "delegated task", WaitMS: 3000})
