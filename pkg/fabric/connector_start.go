...
func newConnectorStartPlan(root, actionID string) ConnectorStartPlan {
	...
	inner := []string{
		...
		"marker=$(head -n 1 \"$root/bootstrap.complete\")",
		"[ -n \"$marker\" ] || fail 'connector bootstrap marker empty'",
		"case \"$marker\" in \"$action_id \"*) ;; *) fail 'connector bootstrap identity mismatch' ;; esac",
		"[ -n \"${marker#\"$action_id \"}\" ] || fail 'connector bootstrap marker has no version'",
		"expected=\"$root/dune --config $root/config.yaml fabricd\"",
		"if LC_ALL=C ps -e -o command= | grep -F -x -q -- \"$expected\"; then exit 0; fi",
		...
		"LC_ALL=C ps -p \"$pid\" -o command= | grep -F -x -q -- \"$expected\" || fail 'connector process identity changed during startup'",
	}
