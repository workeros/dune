...
func (d *Engine) submitACP(s *executionStream, message *pb.Message, machine string) {
	...
	if request.Operation != "acp.action" || json.Unmarshal(request.Payload, &action) != nil || (action.Action != "new" && action.Action != "load" && action.Action != "resume" && action.Action != "list" && action.Action != "prompt" && action.Action != "set_config_option" && action.Action != "set_mode" && action.Action != "permission" && action.Action != "elicitation" && action.Action != "cancel") {
