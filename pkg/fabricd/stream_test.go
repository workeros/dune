...
func TestCancelledConnectionRejectsBufferedMessage(t *testing.T) {
	...
	s := &executionStream{Stream: wire.Wrap(inbound), ctx: ctx, engine: engine}
	...
	if err := sent.Send(&pb.Message{Kind: "input", RequestId: "buffered", Data: []byte("must not execute")}); err != nil {
		t.Fatal(err)
	}
	cancel()
