...
var schema = []string{
	...
	`CREATE TABLE dune_operations (id TEXT PRIMARY KEY,request_key TEXT NOT NULL,request_digest TEXT NOT NULL,principal_id TEXT NOT NULL REFERENCES dune_principals(id),identity_namespace TEXT NOT NULL,identity_subject TEXT NOT NULL,runner_id TEXT NOT NULL REFERENCES dune_runners(id),fabric_id TEXT NOT NULL,binding_revision BIGINT NOT NULL CHECK(binding_revision>0),action TEXT NOT NULL CHECK(action IN ('create','renew','destroy')),created_at BIGINT NOT NULL,finished_at BIGINT NOT NULL DEFAULT 0,finished BOOLEAN NOT NULL DEFAULT FALSE,outcome TEXT NOT NULL DEFAULT '' CHECK(outcome IN ('','unknown','timed_out','succeeded','failed')),worker TEXT NOT NULL DEFAULT '',execution_revision BIGINT NOT NULL DEFAULT 0 CHECK(execution_revision>=0),lease_until BIGINT NOT NULL DEFAULT 0,exclusive BOOLEAN NOT NULL DEFAULT TRUE,UNIQUE(principal_id,request_key),CHECK(finished=FALSE OR exclusive=FALSE),CHECK((finished=FALSE AND finished_at=0 AND outcome IN ('','unknown','timed_out')) OR (finished=TRUE AND finished_at>0 AND outcome IN ('succeeded','failed'))))`,
	...
	`CREATE TABLE dune_managed_destroy_closures (operation_id TEXT NOT NULL REFERENCES dune_managed_destroys(operation_id) ON DELETE CASCADE,instance_id TEXT NOT NULL,machine_id TEXT NOT NULL,acknowledged_at BIGINT NOT NULL DEFAULT 0,PRIMARY KEY(operation_id,instance_id))`,
	`CREATE INDEX dune_managed_destroy_closures_pending ON dune_managed_destroy_closures(instance_id,acknowledged_at)`,
