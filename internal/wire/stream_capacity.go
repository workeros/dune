...
func RequestClass(m *pb.Message) StreamClass {
	...
	case "runtime.watch":
		return StreamWatch
	case "runner.upgrade.preview", "runner.upgrade.get", "runner.upgrade.list", "runner.upgrade.probe", "runner.upgrade.inspect", "machine.info", "runtime.list", "runtime.get", "acp.state", "acp.raw.state", "acp.raw.read", "acp.conversation.read", "acp.conversation.get", "agent.operation.read", "runtime.capture", "runtime.scrollback", "profile.status":
		return StreamRead
	...
	case "submission.acp", "runtime.stop", "runtime.forget":
		...
		if action.Action == "elicitation" && api.ValidateSubmissionID(action.ElicitationID) == nil && action.ElicitationResponse != nil {
			return StreamElicitation
		}
