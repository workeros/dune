...
import { bindingKey, errorText, listAll, request, runtimeTitle, type ProfileRecord, type Runner } from "../lib/api";
...
export function ParallelWorkbench({ accountID, runners, runnersLoading, selectedRunner, onSelectRunner, profileVersion, onManageProfiles }: {
  ...
  const paneBody = (leaf: Leaf) => {
    ...
    return <div className="agent-pane-card"><header>
      <button className="pane-title" onClick={() => focus(leaf.id)} title={`${projectName} · ${runner?.name ?? leaf.pane.target.binding.runner_id}`}><strong>{runtimeTitle(agent?.runtime, leaf.pane.target.runtime.adapter.toUpperCase())}</strong><small>{projectName} · {runner?.name ?? "原环境"}</small></button>
      <span className="pane-status">{agent ? activityLabel(agent.runtime) : "待核验"} · {leaf.pane.target.runtime.adapter.toUpperCase()}</span>
      <Button size="sm" variant="ghost" aria-label={`固定审阅 ${runtimeTitle(agent?.runtime, leaf.id)}`} onClick={() => { saved.change((old) => ({ ...old, review_pane: old.review_pane === leaf.id ? undefined : leaf.id })); setReview((old) => old || "git"); }}>{saved.view.review_pane === leaf.id ? "取消固定" : "固定审阅"}</Button>
      {bindingValid && runner.online && agent && ["running", "exited", "lost"].includes(agent.runtime.state) && <Button size="sm" variant="ghost" aria-label={`${agent.runtime.state === "running" ? "停止" : "删除"} ${runtimeTitle(agent.runtime, leaf.id)}`} onClick={() => setStopping(agent)}>{agent.runtime.state === "running" ? "停止" : "删除"}</Button>}
      <Button size="sm" variant="ghost" aria-label={`移出 ${runtimeTitle(agent?.runtime, leaf.id)}`} title="移出视图，Agent 继续运行" onClick={() => saved.change((old) => removePane(old, leaf.id))}>移出</Button>
  ...
  return <div className="parallel-workbench">
    ...
    <div className="parallel-body"><aside className="agent-navigation">
      ...
      <nav aria-label="Agent 列表">{visibleAgents.map((agent) => {
        ...
        return <button key={targetKey(agent.target)} className={`agent-nav-row ${opened?.id === saved.view.focus_pane ? "selected" : ""}`} disabled={!saved.loaded} onClick={() => open(agent)} aria-label={`打开 ${runtimeTitle(agent.runtime, agent.runtime.adapter)} · ${agent.runner.name}`}>
          <strong>{runtimeTitle(agent.runtime, agent.runtime.adapter)}{read.unread(agent) && <span className="unread-mark" aria-label="未读活动">●</span>}</strong><span>{activityLabel(agent.runtime)} · {agent.runtime.adapter.toUpperCase()}{opened ? " · 已打开" : ""}</span><small>{association?.project.name ?? "未归类"} · {agent.runner.name}</small>
        </button>;
