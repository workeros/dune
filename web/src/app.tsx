...
import { APIError, request, post, call, errorText, type Page, type User, type Tenant, runnerPath, bindingKey, runtimeKey, type Binding, type Runner, type BoundRunner, type Runtime, type AgentConfig, type StartupInfo, type ManagedField, type ManagedTemplate, type ManagedUnavailableReason, type ManagedOperation, type ManagedCreation } from "@/lib/api";
...
export function App() {
  ...
  const [tenants, setTenants] = useState<Tenant[]>([]), [tenantID, setTenantID] = useState(""), [tenantsLoading, setTenantsLoading] = useState(true);
  ...
  useEffect(() => { request<User>("/api/v1/me").then((authenticated) => { setTenantsLoading(true); setUser(authenticated); }).catch((e) => { setUser(null); if (!(e instanceof APIError && e.status === 401)) setError(errorText(e)); }); }, []);
  const loadTenants = async () => { setTenantsLoading(true); try { const result = await request<{ items: Tenant[] }>("/api/v1/tenants"); setTenants(result.items); setTenantID((current) => result.items.some((tenant) => tenant.id === current) ? current : result.items[0]?.id ?? ""); } finally { setTenantsLoading(false); } };
  useEffect(() => {
    if (!user) { setTenants([]); setTenantID(""); setTenantsLoading(false); return; }
    if (!startup) return;
    if (!startup.tenant_scoped) { setTenants([]); setTenantID(user.id); setTenantsLoading(false); return; }
    void loadTenants().catch((e) => setError(errorText(e)));
  }, [user?.id, startup?.tenant_scoped]);
  const refresh = async () => {
  ...
  if (!user) return <Auth startup={startup} onLogin={(user) => { discoveryEpoch.current++; setDiscoveryLoading(true); setTenantsLoading(true); setRunners([]); setSelected(undefined); setPageCursor(""); setNextCursor(""); setPreviousCursors([]); setUser(user); setError(""); }} initialError={error} />;
  if (tenantsLoading) return <main className="grid min-h-screen place-items-center paper-grid"><DesertAtmosphere /><span role="status">正在读取租户…</span></main>;
  if (startup.tenant_scoped && !tenants.length) return <TenantSetup canCreate={!!user.email} onCreated={loadTenants} />;
  const scopedRunners = startup.tenant_scoped ? runners.filter((runner) => runner.tenant_id === tenantID) : runners;
  const current = scopedRunners.find((r) => r.id === selected?.id);
  ...
  const renderWorkbench = () => {
    if (workspace) return <Workspace key={bindingKey(workspace.binding)} runner={workspace} onRefresh={refresh} onRevoke={async () => { setSelected(undefined); await refresh(); }} />;
    if (discoveryLoading && !scopedRunners.length) return <div className="m-auto p-8 muted" role="status">正在加载环境…</div>;
    ...
    if (scopedRunners.length > 0) return <section className="workspace-prompt paper-card m-auto p-7"><div className="mb-5 inline-flex rounded-xl border border-foreground bg-yellow/40 p-3"><Icon icon={serverIcon} width={25} /></div><h1 className="mb-3 text-2xl font-bold">选择一个开发环境</h1><p className="muted leading-7">从左侧列表选择环境，查看或继续其中的工作。</p></section>;
...
function managedRequestBody(template: ManagedTemplate, name: string, credentialEmail: string, values: Record<string, string | boolean>, requestKey: string): string {
  ...
  const credential = credentialEmail ? `,"credential_email":${JSON.stringify(credentialEmail)}` : "";
  return `{"request_key":${JSON.stringify(requestKey)},"request":{"name":${JSON.stringify(name)},"fabric_id":${JSON.stringify(template.fabric_id)},"template_id":${JSON.stringify(template.id)},"template_version":${JSON.stringify(template.version)}${credential},"parameters":{${parameters.join(",")}}}}`;
}
...
function AddManaged({ tenantID, requireCredentialEmail, open, onOpenChange, onCreated }: { tenantID: string; requireCredentialEmail: boolean; open: boolean; onOpenChange: (open: boolean) => void; onCreated: (runner: Runner) => Promise<void> }) {
  const [templates, setTemplates] = useState<ManagedTemplate[]>([]), [templateKey, setTemplateKey] = useState(""), [name, setName] = useState(""), [credentialEmail, setCredentialEmail] = useState(""), [values, setValues] = useState<Record<string, string | boolean>>({}), [error, setError] = useState(""), [busy, setBusy] = useState(false), [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    request<{ items: ManagedTemplate[] }>(`/api/v1/managed/tenants/${encodeURIComponent(tenantID)}/templates`).then(({ items }) => {
      if (!alive) return;
  ...
  const close = (value: boolean) => {
    onOpenChange(value);
    if (!value) { setName(""); setCredentialEmail(""); setValues({}); setError(""); requestKey.current = managedRequestKey(); }
  };
  const submit = async (event: FormEvent) => {
    ...
    try {
      const body = managedRequestBody(selected, name, credentialEmail, values, requestKey.current);
      const created = await request<ManagedCreation>(`/api/v1/managed/tenants/${encodeURIComponent(tenantID)}/runners`, { method: "POST", body });
      close(false); await onCreated(created.runner);
    } catch (e) { setError(errorText(e)); } finally { setBusy(false); }
  };
  return <Dialog open={open} onOpenChange={close}><DialogContent><DialogTitle className="mb-2 text-xl font-bold">创建托管环境</DialogTitle><DialogDescription className="muted mb-6">选择管理员提供的模板。提交后可以离开页面，创建会在后台继续。</DialogDescription>{loading ? <p role="status">正在读取可用模板…</p> : templates.length ? <form className="grid gap-4" onSubmit={submit}><label>环境名称<Input value={name} onChange={(e) => setName(e.target.value)} required maxLength={120} placeholder="例如：项目开发环境" disabled={!selected?.available} /></label>{requireCredentialEmail && <label>Codebase 凭据邮箱<Input type="email" value={credentialEmail} onChange={(e) => setCredentialEmail(e.target.value)} required maxLength={256} placeholder="已向此 Service Account 授权的邮箱" /></label>}<label>模板<select value={templateKey} onChange={(e) => setTemplateKey(e.target.value)}>{templates.map((template) => { const key = JSON.stringify([template.fabric_id, template.id, template.version]); return <option key={key} value={key} disabled={!template.available}>{managedTemplateLabel(template)}</option>; })}</select></label>{selected && !selected.available && <div className="error-box" role="status">{selected.unavailable_reason ? managedUnavailableReason[selected.unavailable_reason] : managedUnavailableReason.unknown}，当前不能创建新环境。</div>}{selected?.fields.map((field) => <ManagedInput key={field.name} field={field} value={values[field.name]} disabled={!selected.available} onChange={(value) => setValues((current) => ({ ...current, [field.name]: value }))} />)}{error && <div className="error-box" role="alert">{error}</div>}<Button type="submit" disabled={busy || !selected?.available}>{busy ? "提交中…" : "创建环境"}</Button></form> : !error && <p className="muted">当前账号没有可用的托管模板。</p>}{error && !templates.length && <div className="error-box" role="alert">{error}</div>}</DialogContent></Dialog>;
}
...
function Workspace({ runner, onRefresh, onRevoke }: { runner: BoundRunner; onRefresh: () => Promise<void>; onRevoke: () => Promise<void> }) {
  const [sessions, setSessions] = useState<Runtime[]>([]), [selected, setSelected] = useState<Runtime>(), [configs, setConfigs] = useState<AgentConfig[]>([]), [configID, setConfigID] = useState(""), [cwd, setCwd] = useState(""), [error, setError] = useState(""), [busy, setBusy] = useState(false), [editing, setEditing] = useState(false), [browsing, setBrowsing] = useState(false), [tab, setTab] = useState<"terminal" | "diff">("terminal"), [revoking, setRevoking] = useState(false), [managedStatus, setManagedStatus] = useState<ManagedOperation>(), [stateBusy, setStateBusy] = useState(false);
  ...
  return <><header className="work-header"><div><h1 className="flex items-center gap-2 text-lg font-bold"><Icon icon={serverIcon} />{runner.name}</h1><p className="mt-1 text-xs text-muted-foreground"><i className={`status-dot ${runner.online ? "online" : ""}`} />{managedStatus?.access_suspended ? managedStage[managedStatus.stage ?? ""] ?? "访问已暂停" : runner.online ? "已连接开发机" : "开发机离线，历史暂不可访问"} · {runner.os}/{runner.arch}</p></div><div className="flex gap-2">{runner.kind === "managed" && managedStatus?.capabilities?.pause_resume && !managedStatus.access_suspended && managedStatus.resource_state === "ready" && <Button variant="outline" disabled={stateBusy} onClick={() => void changeManagedState("pause")}>{stateBusy ? "提交中…" : "暂停环境"}</Button>}<Button variant="outline" onClick={() => setEditing(true)} disabled={!runner.online || managedStatus?.access_suspended}><Icon icon={settingsIcon} />Agent 配置</Button><Button variant="ghost" size="icon" aria-label="解绑开发机" onClick={() => setRevoking(true)}><Icon icon={deleteIcon} /></Button></div></header>
    ...
    <Dialog open={revoking} onOpenChange={setRevoking}><DialogContent><DialogTitle className="mb-3 text-xl font-bold">{runner.kind === "managed" ? "删除" : "解绑"} {runner.name}？</DialogTitle><DialogDescription className="muted mb-6">{runner.kind === "managed" ? "访问会立即关闭，后台将继续确认并删除供应商资源。" : "当前网页连接会被撤销。不会删除开发机文件，也不会终止远端 Agent 或终端进程。"}</DialogDescription><Button variant="destructive" onClick={() => { const action = runner.kind === "managed" ? request(`/api/v1/managed/runners/${encodeURIComponent(runner.id)}`, { method: "DELETE", body: JSON.stringify({ request_key: destroyKey.current }) }) : request(runnerPath(runner.binding, "binding"), { method: "DELETE" }); void action.then(onRevoke).catch((e) => setError(errorText(e))); }}>确认{runner.kind === "managed" ? "删除" : "解绑"}</Button></DialogContent></Dialog>
  </>;
}
