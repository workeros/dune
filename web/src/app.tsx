...
export function App() {
  ...
  useEffect(() => { request<User>("/api/me").then((authenticated) => { setTenantsLoading(true); setUser(authenticated); }).catch((e) => { setUser(null); if (!(e instanceof APIError && e.status === 401)) setError(errorText(e)); }); }, []);
  const loadTenants = async () => { setTenantsLoading(true); try { const result = await request<{ items: Tenant[] }>("/api/tenants"); setTenants(result.items); setTenantID((current) => result.items.some((tenant) => tenant.id === current) ? current : result.items[0]?.id ?? ""); } finally { setTenantsLoading(false); } };
  useEffect(() => {
...
function TenantSetup({ canCreate, onCreated }: { canCreate: boolean; onCreated: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try { await post<Tenant>("/api/tenants", { name }); await onCreated(); }
...
function AddManaged({ tenantID, requireCredentialEmail, open, onOpenChange, onCreated }: { tenantID: string; requireCredentialEmail: boolean; open: boolean; onOpenChange: (open: boolean) => void; onCreated: (runner: Runner) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    request<{ items: ManagedTemplate[] }>(`/api/managed/tenants/${encodeURIComponent(tenantID)}/templates`).then(({ items }) => {
      if (!alive) return;
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const created = await request<ManagedCreation>(`/api/managed/tenants/${encodeURIComponent(tenantID)}/runners`, { method: "POST", body });
...
function Workspace({ runner, onRefresh, onRevoke }: { runner: BoundRunner; onRefresh: () => Promise<void>; onRevoke: () => Promise<void> }) {
  ...
  const loadManagedStatus = async () => { if (runner.kind !== "managed") return; const status = await request<ManagedOperation>(`/api/managed/runners/${encodeURIComponent(runner.id)}`); if (alive.current) setManagedStatus(status); };
