...
export function ACPPane({ accountID, binding, runtime, agentRef, prefix, onNativeChange }: { accountID: string; binding: Binding; runtime: Runtime; agentRef?: string; prefix: string; onNativeChange: () => void }) {
 ...
 const requests = useACPOperations(accountID, prefix, binding, runtime, true, onNativeChange);
 ...
 const act = async (action: string, extra: Record<string, string> = {}) => {
  ...
  try {
   if (action === "prompt" || action === "new" || action === "load") {
    ...
    if (!agentRef) throw new Error("正在同步会话引用，请稍后提交。");
    const submissionID = saveACPSubmission(accountID, prefix, binding, runtime, agentRef, action);
 ...
 return <div className="flex min-h-0 flex-1 flex-col">
  ...
  <ACPSubmissionRecovery accountID={accountID} prefix={prefix} binding={binding} runtime={runtime} />
