...
function storageKey(accountID: string) {
 if (!accountID) throw new Error("无法确认提交记录所属账号。");
 return `dune.acpSubmissions:${JSON.stringify(accountID)}`;
}
...
function writeSaved(accountID: string, items: Saved[]) {
 const raw = JSON.stringify(items);
 if (raw.length > 4 * 1024 * 1024) throw new Error("本地提交记录超过容量，请先移除不再需要的记录。");
 sessionStorage.setItem(storageKey(accountID), raw);
...
export function saveACPSubmission(accountID: string, prefix: string, binding: Binding, runtime: Runtime, agentRef: string, action: string): string {
 if (!agentRef || !Object.hasOwn(actions, action)) throw new Error("提交必须包含准确会话引用。");
 const items = readSaved(accountID);
...
function matchesReceipt(accountID: string, saved: Saved, receipt: Receipt) {
 const target = receipt?.target;
 return receipt?.submission_id === saved.submission_id && ["unknown", "accepted", "not_accepted", "expired"].includes(receipt.admission) && target?.owner_id === accountID && target.runner_id === saved.binding.runner_id && target.fabric_id === saved.binding.fabric_id && target.machine_id === saved.binding.machine_id && target.binding_revision === saved.binding.revision && target.runtime_id === saved.runtime.id && target.runtime_incarnation === saved.runtime.incarnation && target.runtime_generation === saved.runtime.generation;
}
export function recordACPReceipt(accountID: string, id: string, value: unknown) {
 const items = readSaved(accountID), saved = items.find((item) => item.submission_id === id), receipt = value as Receipt;
 if (!saved || !matchesReceipt(accountID, saved, receipt) || receipt.admission === "accepted" && !receipt.operation_ref) throw new Error("提交回执与发送前保存的身份不一致，请查询原提交。");
 writeSaved(accountID, items.map((item) => item === saved ? { ...item, admission: receipt.admission, operation_ref: receipt.operation_ref, stage: receipt.stage, error_code: receipt.error_code, cleanup: receipt.cleanup } : item));
 return receipt;
}
...
export function ACPSubmissionRecovery({ accountID, prefix, binding, runtime }: { accountID: string; prefix: string; binding?: Binding; runtime?: Runtime }) {
 ...
 useEffect(() => {
  const refresh = () => {
   try { setItems(readSaved(accountID).filter((item) => item.prefix === prefix && (!bindingIdentity || bindingKey(item.binding) === bindingIdentity) && (!runtimeIdentity || runtimeKey(item.runtime) === runtimeIdentity))); }
   catch (cause) { setError(errorText(cause)); }
  };
  refresh(); window.addEventListener(changed, refresh);
  return () => window.removeEventListener(changed, refresh);
 }, [accountID, prefix, bindingIdentity, runtimeIdentity]);
 const query = async (saved: Saved) => {
  ...
  try {
   ...
   recordACPReceipt(accountID, saved.submission_id, receipt);
   if (receipt.admission === "accepted" && !controlActions.has(saved.action)) {
    ...
    writeSaved(accountID, readSaved(accountID).map((item) => item.submission_id === saved.submission_id ? { ...item, outcome } : item));
   }
  } catch (cause) { setError(`${errorText(cause)}；保留原提交记录，可以重新查询。`); }
  finally { setReading(undefined); }
 };
 const remove = (id: string) => {
  try { writeSaved(accountID, readSaved(accountID).filter((item) => item.submission_id !== id)); } catch (cause) { setError(errorText(cause)); }
 };
