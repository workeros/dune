...
function readSaved(): Saved[] {
 ...
 if (!Array.isArray(items) || items.length > 576 || items.some((item) => !item || typeof item.submission_id !== "string" || typeof item.agent_ref !== "string" || !item.binding || !item.runtime || !Object.hasOwn(actions, item.action))) throw new Error("本地提交记录损坏，未覆盖原记录。");
...
export function saveACPSubmission(prefix: string, binding: Binding, runtime: Runtime, agentRef: string, action: string): string {
 ...
 const items = readSaved(), control = controlActions.has(action);
 if (items.filter((item) => controlActions.has(item.action) === control).length >= (control ? 512 : 64)) throw new Error("本地提交记录已满，请先移除不再需要的记录。");
...
export function ACPSubmissionRecovery({ prefix, binding, runtime }: { prefix: string; binding: Binding; runtime: Runtime }) {
 ...
 const runtimeIdentity = runtimeKey(runtime), bindingIdentity = bindingKey(binding);
 useEffect(() => {
  const refresh = () => {
   try { setItems(readSaved().filter((item) => item.prefix === prefix && bindingKey(item.binding) === bindingIdentity && runtimeKey(item.runtime) === runtimeIdentity)); }
 ...
 return <details className="border-t px-3 py-2 text-xs" aria-label="提交恢复记录"><summary>提交记录 · {items.length} · 刷新后可查询</summary>
  ...
  <div className="max-h-48 overflow-auto">{items.map((item) => <div className="flex flex-wrap items-center gap-2 border-b py-2" key={item.submission_id}>
   <strong>{actions[item.action]}</strong><time>{new Date(item.created_at).toLocaleTimeString()}</time>
