...
访问凭据新增权威 `owner_id`，升级时从对应 Runner 填充，保留原会话和固定绑定。人类会话的 principal 可在企业检查器明确允许时访问另一 owner 的 Runner；持续有效性检查仍要求原 owner、Runner、Fabric、机器及修订不变，授权不能自动跟随归属或绑定变化。默认 owner 模式的访问范围不变。

新增有界发现游标表，保存不透明 ID、用户、身份源、列表用途、内部位置及原到期时间。游标不授予访问，每页仍执行当前策略；转库和备份恢复完整保留记录，不延长期限。恢复后的查询继续受当前身份和检查器约束。

升级须停止所有旧版本实例并备份，再以一致身份和 AccessChecker 配置启动。浏览器静态资源、服务端与人类 CLI 同步升级：机器和 Runner 列表已改为带 `items`、可选 `next_cursor` 的分页对象，旧数组客户端不兼容。原机器配置、Runner 绑定和远端 Runtime 保留；无需重新安装 fabricd。

## schema 8：本次登录的外部主体

会话与短期访问凭据新增 `identity_subject`；Attached 安装材料新增签发时的 `identity_namespace` 和 `identity_subject`。外部登录保存提供方验证的稳定 subject，本地登录两者为空。CLI 子会话复制父会话的引用，持续访问同时检查父子引用一致及原凭据捕获的 subject。SQL 转库和原生备份完整保存这些字段，不查询当前映射来替换原身份。

从旧 SQL schema 升级时，旧企业会话没有记录具体登录 subject，迁移会撤销这些会话及其 CLI 子会话、已确认 CLI 请求和待消费访问凭据，企业用户须重新登录。旧安装材料也无法确定签发身份源，全部作废并重新签发。迁移不猜测唯一或最近关联身份；原关联、principal、机器身份、Runner、Runtime、本地密码会话及其访问凭据保留，无需重装 fabricd。旧 JSON 本身只支持本地身份，其离线导入仍按本地身份处理。

按原停站备份流程升级所有实例，不混用旧二进制。迁移失败时整个事务回滚；恢复旧备份须遵守前述撤销重放边界。独立 SQLite 重开、PostgreSQL 跨池、schema 7 升级及失败回滚、转库与 PostgreSQL 17 原生恢复由对应回归覆盖。

## schema 9：生命周期操作协调

新增内部 `dune_operations`，保存发起者身份引用、请求键与 SHA-256 摘要、原 Runner/Fabric/绑定修订、动作、结果、业务互斥和 worker 执行租约。相同发起者与请求键只对应一个不可更换的意图；每个 Runner 同时至多一个操作持有业务互斥。worker 领取修订单调增加，租约以数据库时间为准，失效或旧修订不能提交结果。
...
从 schema 9 停站、备份并升级，只新增空表，保留身份、凭据、Runner 和生命周期操作。失败时整体回滚；旧二进制不支持新 schema，回退须恢复协调备份。协议 Gateway 已提供显式目录构造器，官方 host/CLI 集群装配、peer 与自动转发仍未完成；本次迁移不会自动选择目录模式。目录能力与工作台多实例部署不能混为同一个开关。
...
## schema 11：逐请求 peer 用户访问凭据

新增 `dune_peer_access`，保存单次随机凭据的哈希、原会话和机器引用、入口/目标启动身份、身份源、请求摘要、数据库期限与有界授权上下文。上下文保留原用户、subject、授权版本、Runner/Fabric/绑定修订及入口决定 ID；业务正文、原用户 token 和 peer 传输凭据不写入该表。每会话最多六十四个待消费项，期限最多三十秒；签发清理该会话的过期项，删除会话或机器时级联删除。

从 schema 10 协调停站、备份并升级时只新增空表和索引，原用户、登录、Runner、机器及 Runtime 不变。失败时事务回滚，不支持旧二进制与新 schema 混用。SQL 转库和原生备份完整保存新增记录，不重置期限、不把已过期凭据变成新凭据。恢复集群仍须按 schema 10 的流程更换恢复代次并重启所有实例；旧上下文绑定原双方启动身份和原请求中的恢复代次，不能随目录更新自动获得新权限。

本迁移不自动开放 peer HTTP 接口或启用集群。用户访问使用 SQL 一次性能力，不引入上下文签名密钥；生产 peer 传输仍须独立认证并保密。凭据单次消费后，目标实例重新检查当前用户权限及固定绑定，持续流的撤销检查不依赖该凭据继续存在。


## Schema 12：共享配置准入

新增 `dune_instances`，保存启动身份、配置指纹、恢复代次与数据库期限。用户、机器、Runner 与业务操作记录不变；SQLite 只迁移表结构，不启用共享准入。转库与原生备份完整保留实例记录，恢复历史记录不会重新赋予原进程权限。

所有 PostgreSQL 宿主均使用十五秒准入期限和五秒续约。指纹不兼容或恢复代次不符时启动失败；自定义身份/权限宿主须声明非敏感 `ConfigurationVersion`，官方 CLI 使用 `--configuration-version`。接入范围、输入期限及配置版本约定见 [peer 传输接入](peer-transport.md#配置准入与变更)。

升级时停止旧二进制并备份，再迁移到 schema 12 和统一配置后启动。不能把尚未参与准入的旧二进制与新版混跑。关闭宿主不删除尚未到期的实例预约；变更不兼容配置须等待旧预约自然过期，避免迟到输入越过原期限。集群备份恢复仍先停站、旋转恢复代次并隔离旧实例，新启动不会自动替换代次或复活旧 boot。迁移失败保留旧 schema 和业务记录，不能靠直接清空预约表强行切换运行配置。
