...
切换顺序为准备新目录、取得启动闸门并用目标程序预检 ACP 宿主、停止旧 fabricd、更新用户服务路径、启动新版、检查私有启动回执、更新 current、删除安装器拥有的旧 release 目录。协议或固定运行依赖无法核验时，在停止旧服务前拒绝。可用目标程序的 `upgrade-check` 预览 JSON 报告；执行时仍在闸门内重新核验，回滚也使用同一路径。详见 [ACP 升级合同](acp-upgrades.md)。回执必须匹配本次 nonce、程序目录、PID/进程启动标识，且进程仍存活；仅 systemd/launchd 接受启动不算成功。超时或失败保留旧目录供检查，但不提供自动回退。服务 PATH 清除旧 release 和下载临时目录。
