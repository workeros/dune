...
- Gateway 增加不可逆 Drain：连接/流准入与排空状态在同一锁下切换，已有业务流继续收发并沿用原访问、输入与 owner 期限；完成通道等待转发及应用清理回调，空闲控制连接不阻塞。Close 仍立即关闭连接，宿主在所有流回调结束后才释放存储；core 不引入 HTTP、SQL 或产品 worker 知识。
- `App.Shutdown(ctx)` 停止新的 HTTP/管理员工作、公开/peer 连接及已有连接上的新流，等待已接受工作，再完成自有 HTTP 服务器的响应刷新并关闭。没有 deadline 时默认五秒，超时取消剩余工作后返回 context 错误；外部服务器继续由宿主管理。持续终端/订阅属于未完成工作，可信回调必须遵守取消和及时返回约定，预算不强制终止不合作代码。
- `App.Readiness()` 与部署前缀下的 `health/ready`、`health/live` 提供不缓存的粗粒度状态。排空使 ready 返回 503，已有工作仍可服务时 live 返回 200；准入失效/关闭拒绝服务，本机 peer 证书过期拒绝报告可接入。计数仅表示本机 HTTP/管理员工作、连接/流及已确认有效路由，不包含目标 ID，也不宣称外部身份、权限、提供方或全部机器健康。
...
- 内部动作记录在调用前保存独立稳定键、动作参数摘要、原 worker/执行修订及阶段起点。创建、引导、续期、销毁分别使用动作键；续期目标固定为绝对到期时间。只有首次预约的明确提交返回派发资格，重复请求、接管或提交回执丢失均不会重新派发。
- 派发前检查当前执行权和原绑定，接管者只能记录经过适配器核验的原动作事实；旧 worker 不能覆盖新阶段。未决动作还会阻止直接完成 Operation 或释放业务互斥，不能用空 outcome 或租约到期绕过外部未决变更。
- 动作结果、确认的资源关联和 Operation 结果同事务提交。资源在同一 Fabric 内唯一，已知引用不能被更换，首次确认时间不随重复核对重置。创建/引导失败保留已知资源；续期超时不延长期限；删除成功必须包含明确的已删除事实，失败不恢复持久访问许可。
...
### 已完成：Managed 销毁接受与访问关闭事务

- `internal/managed.Service.Destroy` 认证当前浏览器主体，并对权威 Runner 执行 `runner.destroy/managed` 检查。未绑定的 Managed Runner 现在仍把 Fabric 和 binding revision 传给权限检查，但不会伪造可执行 machine binding；Attached Runner 和普通拒绝保持不可区分的 not found。
...
- 新增三个正式 Web/peer 进程、两个本地身份账号和两条真实 fabricd 隧道的 PostgreSQL 回归。机器分别连接 B 与 C，所有用户请求固定进入 A；目录确认两个不同 owner 后才开始业务验证。
