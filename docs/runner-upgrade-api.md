...
在线列表先固定宿主未知提交页，再读取执行端历史，按原 `submission_id` 合并去重。
两次读取之间到达的回执不会从本页移除原提交；本次执行端结果中存在的事实优先，否则
保留首次读取的 `unknown`，不占用活动任务位置。`live` 表示本次执行端查询成功，
不表示列表中的所有提交均已接纳。

在线与离线使用同一游标，按首次持久保留的时间 `started_at`、原 `submission_id` 倒序。
内部提交协议及 `pkg/client.StartUpgrade` 使用 `upgrade.Submission{Request, ReservedAt}`；
宿主从 `Reserve` 的持久回执获取时间，执行端原样保留，接纳期限仍按执行端本地时钟计算。
应用层 `upgrade.Service.StartUpgrade` 和 HTTP 请求仍只接收 `Request`。确认、生成操作 ID
和断线均不改变分页位置。分页不是历史快照；新提交从首页发现，活动任务独立返回。
合并分页同时遵守两个来源的条数与字节边界，避免越过未读取的记录。
