...
日期：2026-09-06。状态：设计提案，尚未实现。本文定义个人 Web 版本之后的产品与扩展边界，不表示当前已支持宿主 SDK、SSO、SQL 元数据、Managed 环境或 Gateway 集群。接口、配置和阶段名称均为拟议契约。
...
| 数据库接入 | 第二层提供 SQLite/PostgreSQL 实现，可注入连接鉴权；领域模块与存储模块共同保证业务事务和迁移 |
...
框架无关的请求处理接口以协议请求和流为输入，由上层决定拒绝、处理或调用底层转发。连接与处理器必须显式装配，不能因未注册处理器而静默绕过业务检查。身份与 Runner 上下文由第二层维护，不通过给 core 增加用户/环境字段实现集成。
...
BindingContext 只描述上层确认的连接角色、目标及必要协议上下文，不包含用户、OIDC 或 Runner。连接来源的认证由接入模块完成；core 校验握手与可信上下文一致。接口须明确并发读写、deadline、取消、Close 及连接所有权，不能只定义一个无法可靠关闭阻塞读写的接口。
...
| IdentityProvider、用户会话与登录事务存储 | 身份模块定义；内置身份或企业实现可替换 |
...
| SQLite/PostgreSQL、数据库连接工厂 | 存储模块提供；领域存储接口由相应消费者定义，数据库鉴权可由应用注入 |
...
Dune 的第二层存储模块实现 SQLite 和 PostgreSQL 后端，企业可复用这些实现并注入连接建立及鉴权机制，不必实现整个存储层。替换领域存储实现时仍需满足同一业务事务契约；第一层不导入 SQL 驱动，也不定义用户或 Runner 存储接口。连接必须提供所选后端所需的事务和条件更新语义；任意 SQL/KV 服务不能仅凭能建立连接就被当作受支持数据库。
...
领域模块提出原子业务操作，存储模块用所选数据库实现其事务边界；不能把各模块的独立提交交给应用启动程序拼接。以下步骤必须由一个业务事务提交，不能让 handler 依次调用多个独立提交的 Store 方法：
...
续期策略与续期执行分离。RenewalPolicy 接收 Runner、提供方事实、已确认到期时间、当前配置和必要的发起者关联，返回是否续期、目标期限或时长、下一次检查时间及原因。企业实现决定这些规则和平台权限；个人入口从配置文件构造默认策略。续期模块不自行推导“用户不活跃应删除”或“断线应重建”。
...
Managed 销毁必须先关闭访问，再请求删除资源。这个流程不复用现有 machine.revoke 的删除记录语义，也不与 `runtime.stop` 混用。

| 阶段/结果 | 生命周期与访问模块行为 |
| --- | --- |
| 接受销毁 | 原子保存 Operation、业务互斥、暂停续期及访问限制，递增访问修订 |
| 等待访问失效 | 通知 owner 关闭旧流，取得匹配当前绑定/访问修订的确认；未确认前不进入资源删除 |
| owner 失联 | 仅在旧连接的有界输入租约已失效、fabricd 能保证拒绝旧输入时继续；否则保持阻塞状态并核对 |
| 删除调用前 | 撤销对应 enrollment 和机器身份，保留资源引用，提交执行阶段后调用适配器 |
| 明确删除成功 | 标记资源已销毁，保留必要操作及幂等记录，解除业务互斥 |
| 明确失败或 unknown | 保留访问限制、已知资源及待清理项，暂停续期，不自动解封或重新创建 |

访问关闭屏障覆盖本地、跨实例、直接 machine target 和既有 attach/上传等流。fabricd 必须在实际接受输入时检查当前绑定及有界输入租约，不能仅依赖数据库中已写入禁用标记或 Gateway 定时关闭 socket。若仅能证明旧 owner 的数据库租约过期，不能据此假定旧连接已停止输入。已受理命令可能继续完成，不宣称回滚。

是否因 Runner 销毁关闭访问、何时推进删除阶段由第二层决定。访问模块将其映射为具体连接/流的失效范围，再调用 core 控制接口并核对确认；fabricd 只执行协议连接身份和有效期约束，不查询 Runner 数据库、不理解销毁原因。

首版失败处理选择继续核对或重试清理，不提供销毁后自动恢复接入。被撤销凭据不会因删除失败而重新生效；需要保留残留资源时由人工在提供方侧核对处理。删除请求已发出后取消等待不恢复访问或续期。
...
owner 使用保守的单调本地期限停止新 stream、关闭旧流及隧道；fabricd 的输入有效期不能因传输延迟而延长，过期后拒绝新输入。协议必须覆盖进程暂停、缓冲输入、网络分区和时钟扰动，满足上一节的访问关闭屏障。数据库恢复备份后更换集群恢复代次、使旧服务身份/epoch 空间失效并要求机器重连，不能让历史路由恢复访问能力。
...
启动拒绝 cluster + SQLite/JSON、SQLite 多实例、缺失必要适配实现、模板越界、不可定向的 peer 地址及不满足事务/租约语义的连接。Secret 从私有文件或部署系统读取，不进入前端配置。某个提供方暂不可达时标记其事实未知或不可用于新建，不阻塞无依赖的 Attached 使用，也不遗失已有资源和操作。
...
| 阶段 | 交付内容 | 完成条件 |
| --- | --- | --- |
| S0 分层、宿主与存储基础 | 分离 core 与 HTTP/WS 接入，提取身份/访问、Web/API 和存储模块；公开组装入口、SQLite/PostgreSQL、连接鉴权注入及 JSON 导入 | core 无业务/框架/SQL 依赖；默认 WS 客户端兼容；官方入口使用同一 SDK；独立封装可复用工作台；两种数据库通过业务事务契约 |
...
| S3 多实例 | PostgreSQL 目录、peer、定向转发、访问屏障、worker 接管和排空 | 客户端与机器固定不同实例；网络分区及旧 worker 迟到调用验证通过 |

S1 在 S0 后推进，S2 复用 S1 的 Runner 与绑定模型；S3 建立在 S0 的连接抽象、事务和模块边界上。单实例与多实例都属于本方案交付范围；S3 完成后还须验收 S1/S2 在集群下的登录、续期、绑定和操作接管。每阶段附带安装、迁移及验证材料，不再等待可选快照等功能后才整理企业交付。
...
| 业务准入位置 | Upgrade 认证不替代请求检查；所有 stream 及持续输入经过模块处理链；缺失处理器不能静默直通；撤销由模块触发 core 关闭流 |
...
| 数据库 | 单实例 SQLite/PostgreSQL 均通过事务、并发冲突和重启恢复；SQLite 多实例启动被拒绝 |
...
| 续期策略 | 浏览器关闭或仍在等待连接时按策略续期；企业策略/个人配置生效；策略错误、明确停止和版本冲突有准确结果 |
| 续期故障 | 多实例无冲突领取，响应丢失先核对到期事实；停机跨过期限不自动重建；不会将未知状态显示为已续期 |
| 销毁屏障 | 本地、peer、直接 machine 和已有流全部失效；分区时不能仅凭 DB 标记继续删除；失败保持引用及限制 |
