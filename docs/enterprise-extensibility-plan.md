...
> 本文是历史演进方案。其中人类 CLI 登录与远程操作入口已经移除；当前产品只从 Web 工作台提供用户操作，Gateway 与 fabricd 仍可独立部署。

日期：2026-09-06。状态：已完成。本文定义产品与扩展边界；各能力是否已经交付以代码、测试和[实施记录](enterprise-implementation.md)为准，不能从本方案的接口、配置或阶段名称推断当前可用性。

当前实现以代码和测试为准，PTY 生命周期见 [tmux 后端](tmux-backend.md)。当前处于初始开发阶段：不维护旧数据迁移、旧 schema 升级或旧接口兼容层，直接实现当前完整模型。本文保留已有 HTTP/WS 部署能力。接入契约以通用身份源、已有开发环境和托管执行环境描述。
...
| `internal/webapp/store.go` 原子 JSON 保存账号、会话与机器 | SQLite/PostgreSQL、当前结构初始化及跨对象业务事务；删除 JSON 存储与导入逻辑 |
...
分层重构本身保持现有 WS + Yamux + 长度前缀 protobuf，不因替换框架而改变消息格式或要求重新实现客户端。后续集群所需的 peer、租约等协议扩展按第 8、11 节统一部署。上层可以替换 HTTP 框架与路由，不能随意改变 Dune 协议并宣称与官方客户端兼容。不为验证抽象额外实现 TCP、QUIC 等传输，也不把默认接入模块变成可动态加载的插件。
...
| 阶段 | 交付内容 | 完成条件 |
| --- | --- | --- |
| S0a 连接与传输拆分 | 分离 core 与 HTTP/WS 接入，定义连接处理器及流所有权，保持现有产品行为 | core 无业务/框架/SQL 依赖；当前默认 WS 客户端可用；取消、背压和关闭符合契约 |
| S0b 模块与应用装配 | 提取身份/访问、Web/API 模块，公开组装入口、有限能力配置及部署前缀 | 官方入口使用同一 SDK；独立宿主复用工作台；按连接隔离身份；自定义前缀下各入口可用 |
| S0c SQL 存储 | 提供 SQLite/PostgreSQL 后端、连接鉴权注入、完整结构初始化及共享业务事务 | 两种数据库通过同一业务事务契约；初始化及跨对象失败完整回滚；重开保留当前结构数据 |
| S1 身份与 Attached | 企业身份适配、浏览器及人类 CLI 登录、短期访问凭据、Runner/绑定模型、配置注入 | 真实身份源完成登录与撤销边界验证；所有入口执行访问检查；Attached 接入与撤销边界准确 |
...
S0a → S0b → S0c 是依次验证的检查点；仅保留 SQL 运行时后端，不增加历史格式转换。S1 在 S0c 后推进，S2 复用 S1 的 Runner 与绑定模型；S3 建立在 S0 的连接抽象、事务和模块边界上。单实例与多实例都属于本方案交付范围；S3 完成后还须验收 S1/S2 在集群下的登录、续期、绑定和操作接管。每阶段附带安装、恢复及验证材料，不再等待可选快照等功能后才整理企业交付。

私有集成应直接验证公开 SDK 是否足够，但企业代码和参考材料不进入公共仓库。首版不通过强制 HTTP v1 来冻结尚未验证的抽象。开发阶段的 Go SDK 接口直接更新调用方和示例，不增加兼容包装。

## 11. 当前结构与恢复

数据库直接创建完整当前结构，不提供旧 JSON 导入、SQLite/PostgreSQL 转换或逐版本 schema 升级。结构初始化在单个事务内执行，PostgreSQL 跨实例初始化通过事务锁串行化；已存在的结构须与当前构建匹配。开发结构变更使用新数据库，不自动修改或删除原数据。

同结构备份与灾难恢复属于运行能力。备份前停止写入和 worker，恢复时保留 Runner、绑定、Operation、动作键和原调度期限；外部已发出的任务可能继续执行，恢复后先查询事实，不能因重启或备份恢复重放 Create。无法确认的操作保留 unknown。数据库恢复不能自动恢复网络连接、运行中的 Agent 或上游资源。

集群恢复须隔离旧实例，核对数据并旋转恢复代次，再以一致配置重新启动所有实例。当前协议端、浏览器资源和 SDK 同步部署，不增加历史版本回退或协商分支。工作台断线保留 tmux；fabricd 重启结束 ACP，资源销毁遵循实际生命周期。
...
| Attached | 无资源 TTL；浏览器关闭和解绑不删除主机、文件或任务；重启后 Runner ID 稳定 |
...
| 初始化与恢复 | 完整当前结构原子初始化、并发建库和同结构备份恢复可验证；结构不匹配不被改写；恢复不重放外部操作 |

实施前固定机器数、活跃流、操作量和可接受延迟，记录 p50/p95/p99、故障恢复时间、撤销延迟与续期调度延迟。数据库连接目录和 Managed 续期分别测量压力；没有业务目标时只报告实测规模，不宣称任意规模可用。

本文件保留目标架构和验收标准；逐功能提交、当前阶段判断、测试范围及真实环境证据统一记录在[实施记录](enterprise-implementation.md)，验证入口见[开发流程](workflow.md)。当前 S0、S1、S2、S3 的仓库实现和实施记录所列验收均已完成，其中 S2 已由独立私有适配器取得真实 Managed 提供方的创建、查询、引导、反向 WS、续期、销毁、响应丢失、重启及故障清理证据。具体采用方的私有身份与权限、三台独立 Linux 主机、PostgreSQL HA 和生产容量仍按其部署范围另行验收，不作为本仓库阶段完成状态的替代或扩张。
