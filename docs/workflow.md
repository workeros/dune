...
Gateway 结构化观测运行 `go test -race ./pkg/gateway ./pkg/host -run 'GatewayEmits|Observation|HostEmitsStructuredAccess' -count=1 -timeout=120s`。覆盖本地与 peer 流、连接/路由生命周期、peer owner/epoch、容量拒绝、回调 panic 隔离，以及宿主慢 sink 的有界队列和丢弃计数。事件不包含业务载荷；测试采集器也不是可靠审计存储。
