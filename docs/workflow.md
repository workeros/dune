...
`TestPostgresClusterNetworkPartitions` 需要专用、可由本机回环地址访问的 PostgreSQL 测试服务器，和正常三进程回归一样使用正式 CLI、测试 CA、真实 fabricd/PTY。测试只在各节点的 PostgreSQL/peer 字节链路上暂存双向流量，不改 SQL 或协议、不关闭 TLS 校验；恢复后释放滞留字节。它验证已受理 Exec 的响应丢失返回 unknown 且不重放、入口 SQL 失联后退出及另一入口使用原 owner、owner SQL 失联后退出及 fabricd 重连到第三节点时 epoch/连接 generation 推进。恢复后的原 PTY 和一次副作用分别独立核对。组合运行 `go test -race ./tests -run 'TestPostgresClusterCLIProcesses|TestPostgresClusterNetworkPartitions|TestPostgresAdmissionSurvivesProcessPause' -count=1 -timeout=240s`；本机代理停滞、单机进程暂停和正常链路是各自范围的证据，不等同于跨主机内核丢包、主机/数据库时钟扰动、Linux 或 Managed worker 的完整故障验收。
