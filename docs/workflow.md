...
按行为所属边界维护测试，避免每一层重复完整的成功和失败矩阵：

| 位置 | 主要覆盖 |
| --- | --- |
| `internal/`、`pkg/` 内的测试 | 模块规则、输入校验、状态变化及该层的协议和鉴权边界 |
| `tests/` | CLI/SDK 装配、真实子进程、服务重启和跨连接行为 |
| `im/` 内的测试 | 消息路由、持久化、平台适配及本地假服务整链 |
| `web/src/**/*.test.mjs` | 前端纯状态逻辑、分页合并和本地记录 |
| `web/e2e/` | 浏览器交互、请求组装、刷新恢复和界面反馈 |

同一契约只因操作名、输入或故障点不同而重复时，优先合成带名称的子用例，
共享准备与断言。鉴权、结果未知、请求不重放、容量限制，以及原始/托管 ACP
覆盖不同风险，不能只因代码相似而删除。独立场景保持独立状态；重启前后的
连续性在一个场景内验证。

已删除的旧命令、旧接口不再维护缺席清单；不为固定表数量、完整列顺序或
底层时钟递增单独保留测试。数据库应检查事务、约束、读写和身份隔离等行为。
删除重复用例时，将独有断言移入保留的场景，避免只减少测试入口数量。

`tests/` 和 `pkg/fabricd` 的 `mockACPBinary` 在包内按需编译一次 mock Agent，
由 `TestMain` 清理可执行文件；用例仍分别创建进程、工作目录和会话状态。
