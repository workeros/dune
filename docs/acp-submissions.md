...
managed ACP 普通操作结果为每 Runtime 64 条、每条最多 512 KiB／1024 条更新，完成后最多保留
15 分钟；排队上限为 32。结果表满时拒绝新增普通操作，不提前淘汰未到期结果。
到期由原宿主的完成时间判定，fabricd 重启不重置期限；运行中／排队记录不按完成
TTL 回收。队列或结果容量拒绝使用 `SUBMISSION_CAPACITY_EXHAUSTED`，detail 区分类别，
已能持久保存拒绝时原键返回不可反转的 `not_accepted`。当前最小接纳证据不自动回收，容量不足限制新普通工作，
已有键和控制预留保留。内部回执读取另限 8 并发、状态读取 16、模型分页读取 8，
操作长轮询另限 16，不占用状态读取槽。

`machine.info.submission_capacity` 使用独立注册库的单条只读快照，公开普通键总量及
claimed／accepted／rejected 分类、各类控制的未消费预留、已占键、已接纳、已完成
用量和硬上限。`completed` 已包含于 `accepted` 和 `used`，不是额外可借用的空间。
它统计已终结的控制阶段（也包含 `input_unrecoverable`），不单独证明业务成功。
Runtime 预留直到 forget 完成才释放，因此 `runtimes.used` 包含尚未清理的已退出
Runtime；`runtime_records` 则继续计入保留身份。
`acp.state.resources` 来自原宿主，返回队列和待回答权限的用量／上限、操作结果的
记录数、未完成／完成数、输出字节和更新条数，以及最早完成时间和下一次到期时间。
诊断不接纳新提交、不清除拒绝证据，也不将注册库读取错误解释成零用量。
PTY 投递结果保留既有策略：达到 64 条时可淘汰最早完成的记录，不淘汰仍在执行或排队的投递。
