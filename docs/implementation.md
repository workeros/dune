...
默认 HTTP tunnel 仍拒绝 peer 角色，官方工作台/CLI 未启用集群。生产 peer 传输认证与保密、宿主配置、配置指纹和三节点故障验收须继续接入；不能把测试 transport 或目录构造器单独部署后当作完整 HA 服务。协议核心不导入 SQL 或产品身份，第二层提供同一元数据后端的目录、用户上下文和可信连接入口。
