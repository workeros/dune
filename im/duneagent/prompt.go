...
func (b Backend) Prompt(ctx context.Context, conversation channel.ConversationSession, session channel.AgentSession, request channel.PromptRequest, emit func(channel.AgentEvent) error) (string, error) {
	input := request.Text
	if err := api.ValidateSubmissionID(request.SubmissionID); err != nil {
	...
	operation, err := connection.Submit(ctx, runtime, host.AgentAction{SubmissionID: request.SubmissionID, ExpectedConversationID: session.ConversationID, Action: "prompt", Text: input, SessionID: session.ACPSessionID})
	if err != nil {
